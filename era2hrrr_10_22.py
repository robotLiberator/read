...
def tohrrr(year, month, day):
    ...
    #他只能切二维的
    with h5.File('/mnt/nfs/samba/数据聚变/气象数据/hrrr区域划分实验数据/hrrr_440_24var_lb_mountain/era5_h5/'+year+'/'+month+'/'+day+'.h5') as data:
        for h in range(24):
            for var in range(24):
        ...
        output_file_path = '/mnt/nfs/samba/数据聚变/气象数据/hrrr区域划分实验数据/hrrr_440_24var_lb_mountain/era5_440/'+year+'/'+month.zfill(2)+'/'
        ...
        with h5.File(output_file_path, 'w') as h5_file:
            ...
            h5_file.create_dataset('fields', data =  outdata) 
