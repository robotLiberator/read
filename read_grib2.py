...
father_path = '/mnt/nfs/samba/数据聚变/气象数据/hrrr_grib'


def grib2h5(year, month, day):
    #创建一个形状为(48, 24, 440, 408)的全0数组
    outdata = np.zeros((24, 24, 440, 408), dtype=np.float32)  # 指定数据类型为float32
    for hour in range(24):
        
        # file_path = father_path + '/'+year+'/'+month+'/'+day+'/hrrr.t00z.wrfprsf'+str(hour+1).zfill(2)+'.grib2'
        file_path = father_path + '/'+year+'/'+month+'/'+day+'/hrrr_'+year+month+day+'_hrrr_t'+str(hour).zfill(2)+'z_wrfprsf00.grib2'
    ...
    # output_file_path = '/mnt/nfs/samba/数据聚变/气象数据/hrrr_440_24var_rb/test/'+year+'/'+month.zfill(2)+'/'
    output_file_path = '/mnt/nfs/samba/数据聚变/气象数据/hrrr区域划分实验数据/hrrr_440_24var_lb_mountain/test/'+year+'/'+month.zfill(2)+'/'
